...
struct figure {
	char type; vector2D<double> p1, p2; double r;
};
...
int main () {
	...
	char c; vector<figure> figs(15);
	int n = 1;
	while (cin >> c) {
		if (c == 'r') {
			figs[n].type = c;
			cin >> figs[n].p1 >> figs[n].p2;
			n++;
		} else if (c == 'c') {
			figs[n].type = c;
			cin >> figs[n].p1 >> figs[n].r;
			n++;
		} else {
			// solve
			vector2D<double> pt;
			int ptc = 1;
			while (cin >> pt) {
				if (pt.x == 9999.9 and pt.y == 9999.9) break;
				bool ok = 0;
				for (int i = 1; i < n; ++i) {
					if (figs[i].type == 'r') {
						if (min(figs[i].p1.x,figs[i].p2.x) <= pt.x and pt.x <= max(figs[i].p1.x,figs[i].p2.x) and 
							min(figs[i].p1.y,figs[i].p2.y) <= pt.y and pt.y <= max(figs[i].p1.y,figs[i].p2.y)) {
							cout << "Point " << ptc << " is contained in figure " << i << endl;
							ok = 1;
						}
					} else {
						vector2D<double> v = pt-figs[i].p1;
						if (v*v <= figs[i].r*figs[i].r) {
							cout << "Point " << ptc << " is contained in figure " << i << endl;
							ok = 1;
						}
					}
				...
				if (not ok)
					cout << "Point " << ptc << " is not contained in any figure" << endl;
				ptc++;
			}
		}
	}
