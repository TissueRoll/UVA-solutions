...
int main () {
	...
	vector2D<double> A, B, C, vAB, vAC, mAB, mAC, vpAB, vpAC, c, r;
	vector3D<double> L1, L2, X;
	while (cin >> A >> B >> C) {
		vAB = B-A;
		vAC = C-A;
		mAB = (A+B)/2.0;
		...
		vpAB = vAB.perp();
		vpAC = vAC.perp();
		L1 = vector3D<double>(mAB.x,mAB.y,1)%vector3D<double>(vpAB.x,vpAB.y,0);
		L2 = vector3D<double>(mAC.x,mAC.y,1)%vector3D<double>(vpAC.x,vpAC.y,0);
		X = L1%L2;
		if (abs(X.z) < EPS) {
			cout << 0.00 << endl;
		} else {
			c = vector2D<double>(X.x/X.z,X.y/X.z);
			r = B-c;
			cout << 2.0*PI*sqrt(r*r) << endl;
		}
	}
