...
int main () {
	...
	while (T--) {
		int n; cin >> n;
		vector<vector2D<double>> pts(4*n);
		vector2D<double> pt; double w, h, a, rects = 0; 
		for (int i = 0; i < n; ++i) {
			cin >> pt >> w >> h >> a;
			rects += w*h;
			a = -a/180.0*PI;
			vector2D<double> v0(-w/2.0,-h/2.0);
			vector2D<double> v1(w/2.0,-h/2.0);
			vector2D<double> v2(-w/2.0,h/2.0);
			vector2D<double> v3(w/2.0,h/2.0);
		...
		vector<vector2D<double>> hull = convex_hull(pts);
		double ans = area2D(hull);
		cout << rects/ans*100.0 << " %\n";
	}
