...
int main () {
	...
	vector<vector<vector2D<double>>> kingdoms;
	...
	while (cin >> n) {
		...
		for (int i = 0; i < n; ++i)
			cin >> v[i];
		kingdoms.push_back(v);
	}

	vector<vector<vector2D<double>>> kingdom_walls;
	for (auto k : kingdoms)
		kingdom_walls.push_back(convex_hull(k));

	vector<bool> dead(kingdom_walls.size(),0);

	int x, y;
	while (cin >> x >> y) {
		vector2D<double> ref(x,y);
		for (int i = 0; i < kingdom_walls.size(); ++i) {
			if (convex2D_tests(kingdom_walls[i], 1, ref) != -2)
				dead[i] = 1;
		}
	}
